...
/** Saved Rabbit callback metadata delegating to the current plain singleton. */
public final class AddedRabbitListenerAdapter {
    static final String RABBIT = "Lorg/springframework/amqp/rabbit/annotation/RabbitListener;";
    private static final String TARGET = InjectedNames.PREFIX + "target";
    private static final Set<String> OPTIONS = Set.of("id", "queues", "containerFactory", "concurrency");
    ...
    private AddedRabbitListenerAdapter() { }
    ...
    static Plan inspect(byte[] bytes, Set<String> added) {
        ...
        for (var m : node.methods) {
            ...
            var annotation = m.visibleAnnotations.stream().filter(a -> a.desc.equals(RABBIT)).findFirst().orElse(null);
            ...
            try {
                ...
                for (var a : m.visibleAnnotations) if (!a.desc.equals(RABBIT) && !a.desc.equals("Ljava/lang/Deprecated;"))
                ...
                if (options.containsKey("concurrency")) {
                    ...
                    if (limits.length > 2 || Integer.parseInt(limits[0]) <= 0
                            || (limits.length == 2 && Integer.parseInt(limits[1]) < Integer.parseInt(limits[0])))
                        throw new IllegalArgumentException("concurrency must be a positive number or ascending range");
                }
    ...
    static Delegate create(Class<?> owner, Supplier<Object> current, Plan plan) throws Throwable {
        if (!plan.refused().isEmpty()) throw new IllegalArgumentException("invalid Rabbit listener plan");
        var lookup = LookupCapture.get(owner);
        if (lookup == null) throw new IllegalStateException("no captured lookup for " + owner.getName());
        String internal = Type.getInternalName(owner), generated = internal + "$$ReclazzRabbit";
