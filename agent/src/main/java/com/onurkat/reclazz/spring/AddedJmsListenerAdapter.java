...
/** Saved Jms callback metadata delegating to the current plain singleton. */
public final class AddedJmsListenerAdapter {
    ...
    private AddedJmsListenerAdapter() { }
    ...
    static Plan inspect(byte[] bytes, Set<String> added) {
        ...
        for (var m : node.methods) {
            ...
            var annotation = m.visibleAnnotations.stream().filter(a -> a.desc.equals(JMS)).findFirst().orElse(null);
    ...
    static Delegate create(Class<?> owner, Supplier<Object> current, Plan plan) throws Throwable {
        if (!plan.refused().isEmpty()) throw new IllegalArgumentException("invalid Jms listener plan");
        var lookup = LookupCapture.get(owner);
        if (lookup == null) throw new IllegalStateException("no captured lookup for " + owner.getName());
        String internal = Type.getInternalName(owner), generated = internal + "$$ReclazzJms";
