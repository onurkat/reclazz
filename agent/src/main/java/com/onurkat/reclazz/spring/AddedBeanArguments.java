...
/** Required reference arguments, resolved by the application's own Spring. */
final class AddedBeanArguments {
    private AddedBeanArguments() { }

    static Supplier<Object[]> prepare(Object factory, String beanName, Method metadata) {
        if (metadata.getParameterCount() == 0) return () -> new Object[0];
        try {
            ClassLoader spring = factory.getClass().getClassLoader();
            for (Class<?> type : metadata.getParameterTypes()) requireSupported(type, spring);
            ...
            Method resolve = factory.getClass().getMethod("doResolveDependency", descriptorType, String.class, Set.class, converterType);
            ...
            return () -> {
                ...
                for (int i = 0; i < arguments.length; i++) {
                    try {
                        ...
                        Object descriptor = descriptorType.getConstructor(parameterType, boolean.class).newInstance(parameter, true);
                        descriptorType.getMethod("initParameterNameDiscovery", discovererType).invoke(descriptor, discoverer);
                        arguments[i] = Objects.requireNonNull(resolve.invoke(factory, descriptor, beanName, dependencies, null),
                                "required dependency resolved to null");
    ...
    private static void requireSupported(Class<?> type, ClassLoader spring) throws ClassNotFoundException {
        if (type.isPrimitive() || type.isArray() || Collection.class.isAssignableFrom(type)
                || Map.class.isAssignableFrom(type) || Optional.class == type
        ...
        for (String name : List.of("org.springframework.beans.factory.ObjectFactory", "org.springframework.beans.factory.ObjectProvider"))
