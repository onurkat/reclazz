...
class AddedAsyncEventReloadTest {
    @TempDir Path tmp;

    @ParameterizedTest @ValueSource(booleans={false,true})
    void asyncEventsUseNativeExecutorAndKeepAdmittedBodyAcrossReloadAndRemoval(boolean child) throws Exception {
        ...
        try(var app=builder.jvmArgs("-Dprobe.dir="+tmp).with("App",APP).with("Service",service(0)).start()) {
            ...
            String[] expected={"queued=1 calls=0","captured=true rerouted=true original=true",
                    "refused=true empty=true","rollback=true handler=true queued=1","removed=true captured=true original=true"};
            for(int stage=1;stage<=5;stage++) {
                ...
                assertTrue(app.output().stream().noneMatch(line -> line.contains("receive() was added")), app.tail());
                System.out.println("[async-event] child="+child+" "+app.latest("R"+stage+"="));
            }
        }
    }
    ...
    private static String service(int version) {
        String callback=version==0||version==5?"":"""
                @org.springframework.context.event.EventListener(condition="#p0 != 'skip'")
                @org.springframework.scheduling.annotation.Async("%s")
                ...
                public void receive(String message) throws Exception {
                    ...
                    App.calls.add(message+":v%d:"+Thread.currentThread().getName()+":"+tx+":"+identity());
                    jdbc.update("insert into entries values (?)",message);
                    if(message.equals("fail")) throw new Exception("failure");
                }
                """.formatted(version==1?"one":version==3?"missing":"two",version);
        return """
                package app;
    ...
    private static final String APP="""
            ...
            public class App implements AsyncConfigurer {
                ...
                public org.springframework.aop.interceptor.AsyncUncaughtExceptionHandler getAsyncUncaughtExceptionHandler() {
                    return (failure,method,args)->errors.add(method.getName()+":"+args[0]+":"+failure.getMessage());
                }
                @Bean public org.h2.jdbcx.JdbcDataSource dataSource() {
                    var ds=new org.h2.jdbcx.JdbcDataSource();ds.setURL("jdbc:h2:mem:async_events;DB_CLOSE_DELAY=-1");return ds;
                }
                ...
                public static void main(String[] args) throws Exception {
                    try(var context=new AnnotationConfigApplicationContext(App.class)) {
                        ...
                        for(int stage=1;stage<=5;stage++) {
                            ...
                            if(stage==1) {
                                admitted=current.identity();context.publishEvent("skip");context.publishEvent("queued");
                                result="queued="+ONE.queue.size()+" calls="+calls.size();
                            } else if(stage==2) {
                                ONE.runNext();boolean captured=calls.equals(List.of("queued:v1:one:true:"+admitted));
                                context.publishEvent("fresh");TWO.runNext();
                                boolean rerouted=calls.size()==2&&calls.get(1).equals("fresh:v2:two:true:"+current.identity());
                                ...
                                boolean refused=false;try{context.publishEvent("invalid");}catch(Exception failure){refused=true;}
                                ...
                                context.publishEvent("fail");TWO.runNext();
                                boolean rollback=before==jdbc.queryForObject("select count(*) from entries",Integer.class);
                                last=current.identity();context.publishEvent("last");
                                result="rollback="+rollback+" handler="+errors.equals(List.of("receive:fail:failure"))+" queued="+TWO.queue.size();
                            } else {
                                context.publishEvent("removed");boolean removed=TWO.queue.size()==1;
                                TWO.runNext();
                                boolean captured=calls.size()==4&&calls.get(3).equals("last:v4:two:true:"+last);
                                old.originalAsync();ONE.runNext();
                                result="removed="+removed+" captured="+captured+" original="+(original.equals(List.of("one","one"))&&old.originalTx());
                            }
                            boolean reflected=Arrays.stream(Service.class.getDeclaredMethods()).anyMatch(m->m.getName().equals("receive"));
