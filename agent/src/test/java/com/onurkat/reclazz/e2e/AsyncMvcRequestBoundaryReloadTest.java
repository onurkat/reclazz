...
class AsyncMvcRequestBoundaryReloadTest {
    @TempDir Path tmp;
    ...
    private void run(String kind,boolean child,boolean boundary) throws Exception {
        String server=Arrays.stream(System.getProperty("java.class.path").split(File.pathSeparator))
                .filter(p->{String n=Path.of(p).getFileName().toString();return n.startsWith("tomcat-embed-core-")||n.startsWith("javax.annotation-api-");})
        ...
        try(var app=builder.start()) {
            ...
            System.out.println("[async-mvc] "+kind+" child="+child+" boundary="+boundary+" "+app.latest("WORKER="));
        }
    }
