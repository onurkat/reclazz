...
class ComputedValueFieldReloadTest {
    @TempDir Path tmp;

    @Test
    void computedFieldsUpdateRejectInvalidSavesAndRecoverWithoutRestart() throws Exception {
        ...
        try (var app = WatchedApp.in(tmp).classpath(WatchedApp.springClasspath())
            ...
            probe(app, 1, "8000:25:new:8:4:new:0:same");
            ...
            probe(app, 2, "8000:25:new:8:4:new:0:same");
            ...
            probe(app, 3, "10000:20:recovered:10:5:recovered:0:same");
            ...
            probe(app, 4, "10000:20:recovered:10:5:recovered:0:same");
        }
    }
                    ...
                    try (var context = new AnnotationConfigApplicationContext(App.class)) {
                        ...
                        for (int i = 1; i <= 4; i++) {
                            ...
                            System.out.println("PROBE" + i + "=" + bean.millis + ":" + bean.rate + ":" + bean.label
                                ...
                                + ":" + calls + ":" + (bean == original ? "same" : "replaced"));
                        }
                    }
                }
            }
