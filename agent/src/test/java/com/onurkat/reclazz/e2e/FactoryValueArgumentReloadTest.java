...
class FactoryValueArgumentReloadTest {
    @TempDir Path tmp;

    @Test void factoryArgumentsReloadInOrdinaryLoader() throws Exception { run(false); }
    @Test void factoryArgumentsReloadInChildLoader() throws Exception { run(true); }

    private void run(boolean child) throws Exception {
    ...
    private void probe(WatchedApp app, int stage, String expected) throws Exception {
        ...
        System.out.println("[factory-value] " + actual);
    ...
    private static final String APP = """
            ...
            public class App {
                public static class Client {
                    ...
                    public Client(long millis, int rate) {
                        constructions++;
                        this.millis = millis; this.rate = rate;
                    }
                }
                @Bean
                public Client client(@Value("#{${cfg.seconds} * 1000L}") long millis,
                                     @Value("#{100 / ${cfg.divisor}}") int rate) {
                    return new Client(millis, rate);
                }
