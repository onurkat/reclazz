...
| Computed `@Value` constructor parameter | **Yes**, for supported scalar expressions on directly constructed singletons with one constructor, including [standard transaction-only proxies](docs/usage.md#transaction-proxies-and-property-recreation). Arguments are checked before the bean is rebuilt; existing singleton holders are re-pointed. [Scope and lifecycle](docs/usage.md#computed-value-constructor-parameters) | Yes (restart) | Not verified here |
