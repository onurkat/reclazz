...
| `@Bean` method added after startup | **Yes on a stock JDK**, for direct `@Bean` factories on `@Configuration(proxyBeanMethods=false)`: object return, required reference bean parameters, one unproxied configuration singleton. Multiple names register one singleton with aliases. Direct method `@Primary`/`@Qualifier` metadata participates in Spring argument selection, including parameter qualifiers. Later saves replace the product and update its aliases and selection metadata; removal deletes only Reclazz-owned registrations. Existing names are refused. [Scope and lifecycle](docs/usage.md#bean-methods-added-after-startup) | Yes (restart) | Not verified here |
...
| `@EventListener` refresh | **Yes**, including a method added after startup: direct annotation, one reference event parameter, `void`, on an unproxied singleton. Preserves Spring event filtering, conditions and `@Order`; later saves replace or remove its registration. [Scope and example](docs/usage.md#event-listener-methods-added-after-startup) | Yes (restart) | Yes |
