...
| `@Bean` method added after startup | **Yes on a stock JDK**, for direct `@Bean` factories on `@Configuration(proxyBeanMethods=false)`: object return, required reference bean parameters, one unproxied configuration singleton. Direct method `@Primary`/`@Qualifier` metadata participates in Spring argument selection, including parameter qualifiers. Spring initializes the product. Later saves replace it and update selection metadata; removal deletes only Reclazz-owned registrations. Existing names are refused. [Scope and lifecycle](docs/usage.md#bean-methods-added-after-startup) | Yes (restart) | Not verified here |
