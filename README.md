...
| `@Bean` method added after startup | **Yes on a stock JDK**, for direct `@Bean` factories on `@Configuration(proxyBeanMethods=false)`: no arguments, object return, one unproxied configuration singleton. Spring creates and initializes the product; later saves replace it, and removal deletes only Reclazz-owned registrations. Existing names are refused. [Scope and lifecycle](docs/usage.md#bean-methods-added-after-startup) | Yes (restart) | Not verified here |
