...
| `@Bean` method added after startup | **Yes on a stock JDK**, for direct `@Bean` factories on `@Configuration(proxyBeanMethods=false)`: object return, required reference bean parameters, one unproxied configuration singleton. Spring resolves arguments, including direct parameter `@Qualifier` and primary candidates, and initializes the product. Later saves replace it; removal deletes only Reclazz-owned registrations. Existing names are refused. [Scope and lifecycle](docs/usage.md#bean-methods-added-after-startup) | Yes (restart) | Not verified here |
