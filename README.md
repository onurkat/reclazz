...
| `@Bean` method added after startup | **Yes on a stock JDK**, for direct instance or static `@Bean` factories on `@Configuration(proxyBeanMethods=false)`: object return and one unproxied configuration singleton. Parameters support reference beans, typed collections/maps, reference arrays, `Optional`, Spring providers and direct `@Value`, including primitives. Multiple names register one singleton with aliases. Direct `@Primary`/`@Qualifier` participates in Spring argument selection. Default `@Configuration` also supports overridable, no-argument instance factories: inter-bean calls resolve the container singleton. Static factories retain plain Java call semantics. Later saves replace the product and update its metadata; removal deletes only Reclazz-owned registrations. Existing names are refused. [Scope and lifecycle](docs/usage.md#bean-methods-added-after-startup) | Yes (restart) | Not verified here |
