...
| New service methods with `@PreAuthorize` / `@PostAuthorize` | **Yes for supported classic and modern Spring Security configurations**: native pre/post authorization follows saved policies and old singleton/CGLIB references. Tested with Spring 5.3.39 / Security 5.7.11 and Spring 6.1.14 / Security 6.3.4 (`@EnableMethodSecurity`); unsupported security infrastructure fails explicitly. [Scope](docs/usage.md#authorization-on-added-service-methods) | Yes (restart) | Not verified here |
