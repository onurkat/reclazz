...
| Other methods you add that a framework was going to find (for example, a getter) | **Named.** Added methods live in the companion, which call sites reach and reflection does not, so the scan cannot see them. Reclazz says which method and what will not happen. New `@RequestMapping` methods and the supported `@Bean`/`@Scheduled`/`@EventListener` methods above use adapters; unsupported forms still need a restart | Yes (restart) | Yes |
...
| Add new methods | **Yes** | Reachable from hot-compiled callers; not from reflection on the original class. New Spring MVC endpoints and supported `@Scheduled`/`@EventListener` methods are registered through adapters without a restart |
