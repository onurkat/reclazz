...
| `@EventListener` refresh | **Yes**, including a method added after startup: direct annotation, one reference event parameter, `void` or a single event object return, on a supported singleton (including standard transaction/cache proxies). Supported public callbacks also accept direct transaction/cache annotations. Spring publishes a returned event; null produces none. Preserves filtering, conditions and `@Order`; later saves replace or remove its registration. [Scope and example](docs/usage.md#event-listener-methods-added-after-startup) | Yes (restart) | Yes |
