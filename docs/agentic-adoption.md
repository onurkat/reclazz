...
- [x] Publish the agent jar to Maven Central under a stable coordinate. Live as `com.onurkat.reclazz:reclazz-agent:1.3.0`; the fat agent jar with the Premain manifest (see [publishing.md](publishing.md)). The Spring Boot starter (`reclazz-spring-boot-starter`) and the Maven plugin (`reclazz-maven-plugin`) are on Central too, at the same version. (BUILD + OWNER)
