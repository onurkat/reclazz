...
class MultiClassReloadTest(
    config: TestConfig,
    agentClient: AgentEventClient,
    httpVerifier: HttpVerifier,
) : BaseTest("Multi-class reload", config, agentClient, httpVerifier) {

    override fun run(): TestResult = writeAndVerify(
        targetPath = "${config.srcDir}/com/onurkat/reclazztest/services/TestService.java",
        templateName = "TestService_multiV2.java.txt",
        httpPath = "${config.testEndpointBase}/multi",
        expectedBody = "helper-v2:service-v2",
        additionalWrites = listOf(
            "${config.srcDir}/com/onurkat/reclazztest/services/HelperService.java"
                    to "HelperService_v2.java.txt"
        ),
    )
}
